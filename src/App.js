    ...
    <React.Fragment>
      <AppBar position="static">
        <Toolbar>
          ...
          <Typography
            variant="h6"
            style={{ marginLeft: "10px", float: "center", marginRight: 30 }}
          >
...
const Banner = ({ user, invites, title, setGridView }) => (
  <React.Fragment>
    {user ? (
      <Welcome user={user} invites={invites} setGridView={setGridView} />
    ) : (
      <SignIn />
    )}
  </React.Fragment>
      ...
      if (user) {
        if (snap.val()) {
          ...
          if (snap.val().users[user.uid] && snap.val().users[user.uid].goals) {
            ...
            setGoals(
              Object.values(goals_arr).map((goal) => snap.val().goals[goal])
            );
          } else {
            setGoals({});
          }
          if (
            snap.val().users[user.uid] &&
            snap.val().users[user.uid].invites
          ) {
            let invites_arr = snap.val().users[user.uid].invites;
            setInvite(
              Object.values(invites_arr).map((goal) => snap.val().goals[goal])
            );
          } else {
            setInvite({});
          }
